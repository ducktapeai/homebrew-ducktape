...
for arg in "$@"; do
    if [[ "$arg" == "--skip-test-check" ]]; then
        SKIP_TEST_CHECK=1ip-test-check" ]]; then
        breakTEST_CHECK=1
    fi
doneif [[ "$arg" == "--skip-build" ]]; then
        SKIP_BUILD=1
# Validate arguments
if [[ "$#" -lt 2 ]]; then
    ...
    echo "Options:"Error: Insufficient arguments${RESET}"
    echo "  --skip-test-check    Continue even if tests fail (will still prompt for confirmation)"--skip-build]"
    exit 1Example: ./full-release-process.sh 0.13.5 \"Fixed input handling in notes module\""
fi  echo "Options:"
    echo "  --skip-test-check    Continue even if tests fail (will still prompt for confirmation)"
NEW_VERSION="$1"ip-build         Skip the build and test steps (use when Cargo is not available)"
CHANGELOG_MESSAGE="$2"
...
# Step 1: Check that we're on main branch in both repositories=========${RESET}"
echo -e "\n${YELLOW}Checking git branch status...${RESET}"RSION${RESET}"
cd "$DUCKTAPE_PATH"====================================================${RESET}"
...
if [ "$DUCKTAPE_BRANCH" != "main" ]; then
    ...
    # Check for Git
cd "$HOMEBREW_PATH" git &> /dev/null; then
HOMEBREW_BRANCH=$(git branch --show-current)talled or not in PATH${RESET}"
if [ "$HOMEBREW_BRANCH" != "main" ]; then
    echo -e "${RED}Error: Not on main branch in homebrew-ducktape repository${RESET}"
    exit 1ho -e "${GREEN}Git: OK${RESET}"
fi  fi
    
# Step 2: Check for uncommitted changesnot skipped
echo -e "\n${YELLOW}Checking for uncommitted changes...${RESET}"
cd "$DUCKTAPE_PATH"d -v cargo &> /dev/null; then
if [ -n "$(git status --porcelain)" ]; thennot installed or not in PATH${RESET}"
    echo -e "${RED}Error: There are uncommitted changes in ducktape repository${RESET}"s${RESET}"
    git statusit 1
    exit 1se
fi          echo -e "${GREEN}Cargo: OK${RESET}"
        fi
cd "$HOMEBREW_PATH"
if [ -n "$(git status --porcelain)" ]; theneck as --skip-build flag was provided${RESET}"
    echo -e "${RED}Error: There are uncommitted changes in homebrew-ducktape repository${RESET}"
    git status
    exit 1k for shasum
fi  if ! command -v shasum &> /dev/null; then
...
cd "$DUCKTAPE_PATH"GREEN}shasum: OK${RESET}"
git pull
cd "$HOMEBREW_PATH"
git pulleck for brew
    if ! command -v brew &> /dev/null; then
# Step 4: Update version in Cargo.tomlnot installed or not in PATH${RESET}"
echo -e "\n${YELLOW}Updating version in Cargo.toml to $NEW_VERSION${RESET}"
cd "$DUCKTAPE_PATH"
sed -i '' "s/^version = \".*\"/version = \"$NEW_VERSION\"/" Cargo.toml
    fi
# Step 5: Update CHANGELOG.md
echo -e "\n${YELLOW}Updating CHANGELOG.md${RESET}"ble${RESET}"
cd "$DUCKTAPE_PATH"
...
cd "$DUCKTAPE_PATH"}Checking git branch status...${RESET}"
cargo build --release
echo -e "\n${YELLOW}Running tests (this may take a few minutes)...${RESET}"
if [ "$DUCKTAPE_BRANCH" != "main" ]; then
# Capture test output and status main branch in ducktape repository${RESET}"
...
if [[ -n "$TEST_STATUS" ]]; then
    ...
    if [[ $SKIP_TEST_CHECK -eq 0 ]]; then
        ...
        if [[ ! "$continue_release" =~ ^[Yy]$ ]]; then
            ...
            exit 1"
        fi(git status --porcelain)" ]; then
    else -e "${RED}Error: There are uncommitted changes in ducktape repository${RESET}"
        echo -e "${YELLOW}Continuing despite test failures (--skip-test-check flag was set)${RESET}"
        read -p "Press Enter to continue or Ctrl+C to abort..."
    fi
else
    echo -e "${GREEN}All tests passed successfully!${RESET}"
fi [ -n "$(git status --porcelain)" ]; then
    echo -e "${RED}Error: There are uncommitted changes in homebrew-ducktape repository${RESET}"
...
if git tag -l "v$NEW_VERSION" | grep -q "v$NEW_VERSION"; then
    ...
    if [[ ! "$continue_with_tag" =~ ^[Yy]$ ]]; thento $NEW_VERSION${RESET}"
        read -p "Do you want to force update the tag? (y/n): " force_tag
        if [[ "$force_tag" =~ ^[Yy]$ ]]; thenEW_VERSION\"/" Cargo.toml
            git tag -d "v$NEW_VERSION"
            git tag -a "v$NEW_VERSION" -m "Release $NEW_VERSION: $CHANGELOG_MESSAGE"
            echo -e "${GREEN}Tag v$NEW_VERSION force-updated${RESET}"
        elseE_PATH"
            echo -e "${RED}Release process aborted due to tag conflict${RESET}"SAGE\n\n"
            exit 1HANGELOG_ENTRY/" CHANGELOG.md
        fi
    else: Build and test the project (conditionally)
        echo -e "${YELLOW}Continuing with existing tag v$NEW_VERSION${RESET}"
    fiho -e "\n${YELLOW}Building and testing the project...${RESET}"
elsecd "$DUCKTAPE_PATH"
    git tag -a "v$NEW_VERSION" -m "Release $NEW_VERSION: $CHANGELOG_MESSAGE"
    echo -e "${GREEN}Tag v$NEW_VERSION created${RESET}"few minutes)...${RESET}"
fi
    # Capture test output and status
# Step 9: Push the changes and tagsase -- --nocapture 2>&1) || TEST_STATUS=$?
echo -e "\n${YELLOW}Pushing changes and tags to GitHub...${RESET}"
cd "$DUCKTAPE_PATH"s failed
git push[ -n "$TEST_STATUS" ]]; then
        echo -e "${RED}Tests failed with status code $TEST_STATUS${RESET}"
# Push tags with error handling for already existing tagsPUT\n"
echo -e "\n${YELLOW}Pushing tags to GitHub...${RESET}"
if ! git push --tags 2> /tmp/git_push_error; then
    # Check if error is just about already existing tagsfailed tests
    if grep -q "rejected.*already exists" /tmp/git_push_error; thenss anyway? (y/n): " continue_release
    ...
    else    echo -e "${YELLOW}Continuing despite test failures (--skip-test-check flag was set)${RESET}"
        ...
        if [[ ! "$continue_tag_push" =~ ^[Yy]$ ]]; then
            ...
            exit 1ELLOW}Skipping build and test steps (--skip-build flag was set)${RESET}"
        fi
    fi
elseep 7: Commit the version bump and changelog
    echo -e "${GREEN}Tags pushed successfully${RESET}"log update...${RESET}"
fi "$DUCKTAPE_PATH"
rm -f /tmp/git_push_errorlock CHANGELOG.md
git commit -m "Bump version to $NEW_VERSION: $CHANGELOG_MESSAGE"
# Step 10: Create a tarball and verify SHA256 with GitHub
echo -e "\n${YELLOW}Creating release tarball...${RESET}"
cd "$DUCKTAPE_PATH"}Creating git tag v$NEW_VERSION...${RESET}"
...
echo -e "${GREEN}Tarball created: $RELEASE_TARBALL${RESET}"ists${RESET}"
echo -e "${GREEN}Generated SHA256: $GENERATED_SHA256${RESET}"y/n): " continue_with_tag
...
GITHUB_URL="https://github.com/ducktapeai/ducktape/archive/v$NEW_VERSION.tar.gz"AGE"
            echo -e "${GREEN}Tag v$NEW_VERSION force-updated${RESET}"
# Force clear all Homebrew cache files for this package
HOMEBREW_CACHE_PATH="$HOME/Library/Caches/Homebrew/downloads" conflict${RESET}"
...
if [[ -n "$CACHED_FILES" ]]; then
    ...
    echo "$CACHED_FILES"W}Continuing with existing tag v$NEW_VERSION${RESET}"
    echo -e "${YELLOW}Removing all cached files for ducktape to ensure clean download${RESET}"
    
    for file in $CACHED_FILES; do "Release $NEW_VERSION: $CHANGELOG_MESSAGE"
        rm -f "$file"Tag v$NEW_VERSION created${RESET}"
        echo -e "${GREEN}Removed: $file${RESET}"
    done
    ep 9: Push the changes and tags
    # Also remove any temp files that might be interfering{RESET}"
    rm -f /tmp/ducktape*
    echo -e "${GREEN}Removed any temporary files${RESET}"
fi
# Push tags with error handling for already existing tags
# Thoroughly clean Homebrew cacheto GitHub...${RESET}"
echo -e "${YELLOW}Running brew cleanup to ensure all caches are cleared...${RESET}"
brew cleanup -serror is just about already existing tags
echo -e "${GREEN}Brew cleanup completed${RESET}"it_push_error; then
        echo -e "${YELLOW}Warning: Some tags were rejected because they already exist in the remote.${RESET}"
# Wait a moment for GitHub to fully process the new tagis is normal if you're re-running the release process.${RESET}"
echo -e "${YELLOW}Waiting for GitHub to fully process the new tag (10 seconds)...${RESET}"
sleep 10grep "new tag" /tmp/git_push_error | sed 's/^/  /'
        # Continue with the process despite this "error"
...
fiStep 10: Create a tarball and verify SHA256 with GitHub
echo -e "\n${YELLOW}Creating release tarball...${RESET}"
# Step 11: Update the Formula with correct SHA256 using direct file creation
echo -e "\n${YELLOW}Updating Homebrew formula with verified SHA256...${RESET}"> "$RELEASE_TARBALL"
cd "$HOMEBREW_PATH"shasum -a 256 "$RELEASE_TARBALL" | awk '{print $1}')
echo -e "${GREEN}Tarball created: $RELEASE_TARBALL${RESET}"
# Create the formula file with the correct SHA256256${RESET}"
echo -e "${GREEN}Creating formula with verified SHA256: $SHA256${RESET}"
cat > "$FORMULA_PATH" << EOFownload the GitHub release to ensure we get the real SHA256
class Ducktape < Formulaning Homebrew cache and downloading GitHub release to verify SHA256...${RESET}"
  desc "AI-powered terminal tool for Apple Calendar, Reminders and Notes"
  homepage "https://github.com/ducktapeai/ducktape"archive/v$NEW_VERSION.tar.gz"
  url "https://github.com/ducktapeai/ducktape/archive/v$NEW_VERSION.tar.gz"
  version "$NEW_VERSION"ew cache files for this package
  sha256 "$SHA256"H="$HOME/Library/Caches/Homebrew/downloads"
  license "MIT"find "$HOMEBREW_CACHE_PATH" -name "*ducktape*" 2>/dev/null)
if [[ -n "$CACHED_FILES" ]]; then
  depends_on "rust" => :buildached Homebrew files that may cause SHA mismatch:${RESET}"
    echo "$CACHED_FILES"
  def install${YELLOW}Removing all cached files for ducktape to ensure clean download${RESET}"
    system "cargo", "install", "--root", prefix, "--path", "."
    for file in $CACHED_FILES; do
    # Generate shell completions - with error handling
    begincho -e "${GREEN}Removed: $file${RESET}"
      output = Utils.safe_popen_read(bin/"ducktape", "completions")
      (bash_completion/"ducktape").write output
      (zsh_completion/"_ducktape").write outputinterfering
      (fish_completion/"ducktape.fish").write output
    rescue => eGREEN}Removed any temporary files${RESET}"
    ...
    end "${GREEN}Brew cleanup completed${RESET}"
    
    man1.install "man/ducktape.1" if File.exist?("man/ducktape.1")
  end-e "${YELLOW}Waiting for GitHub to fully process the new tag (10 seconds)...${RESET}"
sleep 10
  test do
    assert_match version.to_s, shell_output("\#{bin}/ducktape --version")
    system "\#{bin}/ducktape", "calendar", "list"d URL for reliable SHA256...${RESET}"
  endT_TARBALL="/tmp/ducktape-direct-$NEW_VERSION.tar.gz"
endELOAD_URL="https://codeload.github.com/ducktapeai/ducktape/tar.gz/refs/tags/v$NEW_VERSION"
EOFcurl -L -s -H "Cache-Control: no-cache" "$CODELOAD_URL" -o "$DIRECT_TARBALL"; then
    DIRECT_SHA256=$(shasum -a 256 "$DIRECT_TARBALL" | awk '{print $1}')
# Verify the formula was created correctlyssful. SHA256: $DIRECT_SHA256${RESET}"
...
elseecho -e "${GREEN}Saved GitHub tarball to /tmp/ducktape-$NEW_VERSION-github.tar.gz${RESET}"
    echo -e "${RED}Error: Formula SHA256 doesn't match expected value!${RESET}"
    echo -e "${RED}Expected: $SHA256${RESET}"tHub. Using locally generated SHA256${RESET}"
    echo -e "${RED}Found: $FORMULA_SHA${RESET}"
    exit 1
fi
# Step 11: Update the Formula with correct SHA256 using direct file creation
# Step 12: Commit and push the formula changeswith verified SHA256...${RESET}"
...
git push"${GREEN}Creating formula with verified SHA256: $SHA256${RESET}"
echo -e "${GREEN}Formula changes pushed to GitHub${RESET}"
class Ducktape < Formula
# Clean up temporary filesl tool for Apple Calendar, Reminders and Notes"
rm -f "$DIRECT_TARBALL"hub.com/ducktapeai/ducktape"
  url "https://github.com/ducktapeai/ducktape/archive/v$NEW_VERSION.tar.gz"
...
brew update
  def install
# Uninstall any previous version to get a clean installh", "."
echo -e "${YELLOW}Uninstalling previous versions if present...${RESET}"
brew uninstall ducktape 2>/dev/null || trueor handling
    begin
# Explicitly tap our repository_read(bin/"ducktape", "completions")
echo -e "${YELLOW}Explicitly tapping the ducktapeai/ducktape repository...${RESET}"
brew untap ducktapeai/ducktape 2>/dev/null || true
brew tap ducktapeai/ducktapetape.fish").write output
    rescue => e
# Force a full re-download to avoid any cached files issuesssage}"
echo -e "${YELLOW}Installing formula from scratch (with build from source)...${RESET}"
brew fetch --force --build-from-source ducktapeai/ducktape/ducktape || {r ducktape\n"
...
# Install with --verbose to see what's happening("man/ducktape.1")
if ! brew install --verbose ducktapeai/ducktape/ducktape; then
    ...
    if ! brew install --verbose --build-from-source ducktapeai/ducktape/ducktape; then
        ...
        echo -e "3. Manually check: ${FORMULA_PATH}"ectly...${RESET}"
        echo -e "4. Try manually: brew fetch --force ducktapeai/ducktape/ducktape".*$/\1/')
        FORMULA_SHA" == "$SHA256" ]]; then
        ...
        if [[ ! "$continue_install" =~ ^[Yy]$ ]]; then expected value!${RESET}"
            echo -e "${RED}Release process aborted due to installation failure${RESET}"
            exit 1}Found: $FORMULA_SHA${RESET}"
        fi
    else
        echo -e "${GREEN}Successfully installed ducktape with --build-from-source!${RESET}"
    fi 12: Commit and push the formula changes
else -e "\n${YELLOW}Committing and pushing Homebrew formula changes...${RESET}"
    echo -e "${GREEN}Successfully installed ducktape!${RESET}"
fit commit -m "Update ducktape formula to version $NEW_VERSION with SHA256 $SHA256"
git push
# Step 14: Verify the installationushed to GitHub${RESET}"
...
if command -v ducktape >/dev/null 2>&1; then
    ...
    ep 13: Installing the formula
    if [[ "$VERSION_OUTPUT" == *"$NEW_VERSION"* ]]; thentall...${RESET}"
        echo -e "${GREEN}Version verification successful!${RESET}"
    elseure we have the latest brew information
    ...
    echo -e "${YELLOW}This could be due to:${RESET}"present...${RESET}"
    echo -e "1. Installation failure"| true
    echo -e "2. The binary not being properly linked"
    echo -e "3. Path issues in your environment"
    echo -e "\n${YELLOW}Try running: brew link --overwrite ducktape${RESET}"RESET}"
fiew untap ducktapeai/ducktape 2>/dev/null || true
brew tap ducktapeai/ducktape
# Step 15: All done!
echo -e "\n${GREEN}=========================================================${RESET}"
echo -e "${GREEN}Release process completed!${RESET}"ith build from source)...${RESET}"
echo -e "${GREEN}Version $NEW_VERSION has been released and Homebrew formula updated.${RESET}"
echo -e "${GREEN}=========================================================${RESET}"}"
echo -e "${YELLOW}Don't forget to:${RESET}"directly which might use the correct SHA from the freshly updated formula.${RESET}"
...
echo -e "  - Announce the release to your users"ducktape; then
    echo -e "${RED}Error: Failed to install ducktape with standard install${RESET}"
...
echo -e "  brew untap ducktapeai/ducktape && brew tap ducktapeai/ducktape"cktape; then
echo -e "  brew install ducktapeai/ducktape/ducktape"n with build-from-source!${RESET}"
...
EOF   ${YELLOW}brew doctor${RESET}5. If you still have issues, try running:   ${YELLOW}brew install --build-from-source ducktapeai/ducktape/ducktape${RESET}4. Install from source:   ${YELLOW}rm -rf ~/Library/Caches/Homebrew/downloads/*ducktape*${RESET}3. Clear any cached downloads:   brew tap ducktapeai/ducktape${RESET}   ${YELLOW}brew untap ducktapeai/ducktape 2>/dev/null || true2. Untap and re-tap the ducktape repository:   ${YELLOW}brew update${RESET}1. First, ensure Homebrew is up to date:ducktape manually using these steps:If the automatic installation did not work, you can install=======================================================${RESET}  MANUAL INSTALLATION INSTRUCTIONS${BLUE}=======================================================cat << EOF# Step 16: Manual Installation Instructions# At the end of the script, add these helpful instructions for manual installation        echo -e "${RED}This suggests there may still be issues with the formula or Homebrew cache.${RESET}"
        echo -e "\n${YELLOW}Troubleshooting suggestions:${RESET}"
        echo -e "1. Run: brew doctor"
        echo -e "2. Run: brew cleanup --prune=all"
